...
const COST_DEPARTMENT_HEADER = '費用單位';
...
function syncPersonnelData(groupKey = 'ICHEF') {
  ...
  const costDepartmentIndex = headerRow.indexOf(COST_DEPARTMENT_HEADER);
  const filteredRows = costDepartmentIndex === -1
    ? dataRows
    : dataRows.filter(function (row) {
        const value = row[costDepartmentIndex];
        return value !== null && value !== undefined && String(value).trim() !== '';
      });

  const output = [headerRow, ...filteredRows];
